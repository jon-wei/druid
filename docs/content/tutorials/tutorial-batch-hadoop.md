...
This Dockerfile and related files are located at `quickstart/hadoop/docker`.
...
Let's create a folder under `/tmp`, we will specify this later when starting the Hadoop container:
...
From the druid-${DRUIDVERSION} package root on the host, copy the `quickstart/wikipedia-2016-06-27-sampled.json.gz` sample data to the shared folder:

```
cp quickstart/wikipedia-2016-06-27-sampled.json.gz /tmp/shared/quickstart/wikipedia-2016-06-27-sampled.json.gz
```
...
./hadoop fs -put /shared/wikipedia-2016-06-27-sampled.json.gz /quickstart/wikipedia-2016-06-27-sampled.json.gz
```
...
```
cp /usr/local/hadoop/etc/hadoop/*.xml /shared/
```

From the host machine, run the following, where {PATH_TO_DRUID} is replaced by the path to the Druid package.

```
cp /tmp/shared/*.xml {PATH_TO_DRUID}/quickstart/conf-quickstart/druid/_common/hadoop-xml/
```

### Update Druid segment and log storage

In your favorite text editor, open `/quickstart/conf-quickstart/druid/_common/common.runtime.properties`, and make the following edits:
...
```bash
curl -X 'POST' -H 'Content-Type:application/json' -d @quickstart/wikiticker-index-hadoop.json http://localhost:8090/druid/indexer/v1/task
```
