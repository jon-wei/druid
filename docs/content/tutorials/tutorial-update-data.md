...
It will also be helpful to have finished [Tutorial: Loading a file](/docs/VERSION/tutorials/tutorial-batch.html), [Tutorial: Querying data](/docs/VERSION/tutorials/tutorial-query.html), and [Tutorial: Rollup](/docs/VERSION/tutorials/rollup.html).
...
The spec we'll use for this tutorial is located at `quickstart/updates-init-index.json`. This spec creates a datasource called `updates-tutorial` from the `quickstart/updates-data.json` input file.

Let's submit that task:

```
bin/post-index-task --file quickstart/updates-init-index.json 
```
...
The `quickstart/updates-overwrite-index.json` spec will perform an overwrite on the `updates-tutorial` datasource.

Note that this task reads input from `quickstart/updates-data2.json`, and `appendToExisting` is set to `false` (indicating this is an overwrite).

Let's submit that task:

```
bin/post-index-task --file quickstart/updates-overwrite-index.json 
```

When Druid finishes loading the new segment from this overwrite task, the "tiger" row now has the value "lion", the "aardvark" row has a different number, and the "giraffe" row has been replaced:
...
Let's try appending some new data to the `updates-tutorial` datasource now. We will add the data from `quickstart/updates-data3.json`.

The `quickstart/updates-append-index.json` task spec has been configured to read from the existing `updates-tutorial` datasource and the `quickstart/updates-data3.json` file. The task will combine data from the two input sources, and then overwrite the original data with the new combined data.

Let's submit that task:

```
bin/post-index-task --file quickstart/updates-append-index.json 
```
...
The `quickstart/updates-append-index2.json` task spec reads input from `quickstart/updates-data4.json` and will append its data to the `updates-tutorial` datasource. Note that `appendToExisting` is set to `true` in this spec.

Let's submit that task:

```
bin/post-index-task --file quickstart/updates-append-index2.json 
```
