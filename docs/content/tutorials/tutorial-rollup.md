...
A file containing this sample input data is located at `quickstart/rollup-data.json`.

We'll ingest this data using the following ingestion task spec, located at `quickstart/rollup-index.json`.
...
```
bin/post-index-task --file quickstart/rollup-index.json 
```
