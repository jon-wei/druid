...
public class ResourceFilterTestHelper
{
  ...
  public void setUp(ResourceFilter resourceFilter) throws Exception
  {
    ...
    authorizationManager = EasyMock.createStrictMock(AuthorizationManager.class);

    // Memory barrier
    synchronized (this) {
      ((AbstractResourceFilter) resourceFilter).setReq(req);
      ((AbstractResourceFilter) resourceFilter).setAuthorizationManager(authorizationManager);
    }
  }
  ...
  )
  {
    ...
    EasyMock.expect(authorizationManager.authorize(
        EasyMock.anyString(),
  ...
  {
    return getRequestPaths(clazz, ImmutableList.<Class<?>>of(AuthorizationManager.class), ImmutableList.<Key<?>>of());
  }
    ...
    final Injector injector = Guice.createInjector(
        new Module()
        {
          ...
          public void configure(Binder binder)
          {
            ...
            binder.bind(AuthConfig.class).toInstance(new AuthConfig(true, null, null));
          }
        }
    );
